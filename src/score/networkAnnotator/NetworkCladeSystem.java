    ...
    private void collectAttributesForClade(BitSet bits, NetworkNode node, Set<String> attributeNames, int segment, Double height) {
        ...
        if (clade == null) {
            ...
            if (clade.attributeValues == null) {
				clade.attributeValues = new HashMap<>();
            }
...
//				clade.attributeValues.put(attributeName, value);
				clade.attributeValues.computeIfAbsent(attributeName, k -> new ArrayList<>()).add(value);
...
//            		clade.attributeValues = new ArrayList<>();
            	...
            	for (int j = 0; j < nrSegments; j++){
            		if (newCoalescentCladeMap.get(j).get(nodeHeights.get(i))!=null){
						for (String key : newCoalescentCladeMap.get(j).get(nodeHeights.get(i)).attributeValues.keySet())
							clade.attributeValues.computeIfAbsent(key, k -> new ArrayList<>()).addAll(
									newCoalescentCladeMap.get(j).get(nodeHeights.get(i)).attributeValues.get(key));
//						clade.attributeValues = (newCoalescentCladeMap.get(j).get(nodeHeights.get(i)).attributeValues);
            		}
            	}
            	clade.setCount(clade.getCount()+1);
            }
...
//            		clade.attributeValues = new ArrayList<>();
            	...
            	for (int j = 0; j < nrSegments; j++){
            		if (segmentDirection.get(i)[j]!=null){
//						clade.attributeValues = newReassortmentCladeMap.get(j).get(nodeHeights.get(i)).attributeValues;
						for (String key : newReassortmentCladeMap.get(j).get(nodeHeights.get(i)).attributeValues
								.keySet())
							clade.attributeValues.computeIfAbsent(key, k -> new ArrayList<>()).addAll(
									newReassortmentCladeMap.get(j).get(nodeHeights.get(i)).attributeValues.get(key));
	        ...
	        for (String attributeName : attributeNames) {
	            switch (attributeName) {
	            	case "height":
						...
						List<Object> rawHeights = cladeMap.get(bitsArray).attributeValues.get("height");
						for (int i = 0; i < rawHeights.size(); i++)
							height.add((double) rawHeights.get(i));
				...
				case "state":
					HashMap<Object, Integer> state = new HashMap<>();
//					List<String> state = new ArrayList<>();
					if (cladeMap.get(bitsArray).count > 1) {
						// can happend when a target network is used
						List<Object> rawStates = cladeMap.get(bitsArray).attributeValues.get("state");
						for (int i = 0; i < rawStates.size(); i++) {
							Object s = rawStates.get(i);
					...
					if (onTarget)
						targetState = node.getTypeLabel();



					if (!onTarget) {
						annotateModeAttribute(node, "state", state);
						annotateFrequencyAttribute(node, "state", state);
					}


//	            		List<Double> type = new ArrayList<>();
//	            		if(cladeMap.get(bitsArray).count>1){
//	            			// can happend when a target network is used
//				    		List<Object[]> rawTypes = cladeMap.get(bitsArray).getAttributeValues();
//				            for (int i = 0; i < rawTypes.size(); i++)
//				            	height.add((double) rawTypes.get(i)[0]);
//	            		}
        ...
        for (String attributeName : attributeNames) {
            switch (attributeName) {
            	case "height":
            		...
            		List<Double> height = new ArrayList<>();
				List<Object> rawHeights = leafCladeMap.get(newBits).attributeValues.get("height");
				for (int i = 0; i < rawHeights.size(); i++)
					height.add((double) rawHeights.get(i));
...
//				List<String> state = new ArrayList<>();
				// can happend when a target network is used
				List<Object> rawStates = leafCladeMap.get(newBits).attributeValues.get("state");
				for (int i = 0; i < rawStates.size(); i++) {
					Object s = rawStates.get(i);
					if (state.containsKey(s)) {
						state.put(s, state.get(s) + 1);
	            ...
	            switch (attributeName) {
	            	case "height":
	            		...
	            		if(reassortmentCladeMap.get(keyArray).count>1){
	            			// can happend when a target network is used
						List<Object> rawHeights = reassortmentCladeMap.get(keyArray).attributeValues.get("height");
						for (int i = 0; i < rawHeights.size(); i++)
							height.add((double) rawHeights.get(i));
	            		}else if(!onTarget){
...
//					List<String> state = new ArrayList<>();
					// can happend when a target network is used
					if (reassortmentCladeMap.get(keyArray).count > 1) {
						List<Object> rawStates = reassortmentCladeMap.get(keyArray).attributeValues.get("state");
						for (int i = 0; i < rawStates.size(); i++) {
							Object s = rawStates.get(i);
							if (state.containsKey(s)) {
								state.put(s, state.get(s) + 1);
	...
	private void annotateModeAttribute(NetworkNode node, String label, HashMap<Object, Integer> values) {
		Object mode = null;
		int maxCount = 0;
		int totalCount = 0;
		int countInMode = 1;
		...
		node.setTypeLabel((String) mode);
		node.setMetaData(node.getMetaData() + "," + label + ".prob=" + freq);
	}

	private void annotateFrequencyAttribute(NetworkNode node, String label, HashMap<Object, Integer> values) {
		double totalCount = 0;
		...
		node.setMetaData(
				node.getMetaData() + "," + label + ".set=" + Arrays.toString(name).replace("[", "{").replace("]", "}"));
		node.setMetaData(node.getMetaData() + "," + label + ".set.prob="
				+ Arrays.toString(freq).replace("[", "{").replace("]", "}"));
	}
...
//            return "count " + getCount();
        ...
        double height;
        boolean isLeft;
		Map<String, List<Object>> attributeValues = null;
    }
