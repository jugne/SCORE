...
/**
 * NOT for general use. Written for a specific case!!
 * 
 * @author Ugne Stolz <ugne.stolz@protonmail.com>
 * @date 25 Jan 2021
 */
public class SegmentRootFilter extends SCoReAnnotator {
    ...
    private static class NetworkAnnotatorOptions {
		...
		File inFile_na;
		File inFile_ha;
		File outFile = new File("networksToRemove.txt");
	...
	public SegmentRootFilter(NetworkAnnotatorOptions options) throws Exception {

        // Display options:
        System.out.println(options + "\n");

        // Initialise reader
		StructuredReassortmentLogReader logReaderNetwork = new StructuredReassortmentLogReader(options.inFile,
				options.burninPercentage);

		System.out.println(logReaderNetwork.getNetworkCount() + " Networks in file.");

		System.out.println("The first " + logReaderNetwork.getBurnin() +
        ...
        try (PrintStream ps = new PrintStream(options.outFile)) {
			...
			boolean first = true;
			int c = 1;
			for (Network network : logReaderNetwork) {
				if (first) {
				...
				Node tree1 = getSingleChildTree(network, 0, leafNodes);
				// get the tree with single child nodes back
				Node tree2 = getSingleChildTree(network, 1, leafNodes);

				String state1 = null, state2 = null;

				for (NetworkNode node : network.getNodes()) {
					if (node.getHeight() == tree1.getHeight())
						state1 = node.getTypeLabel();
					if (node.getHeight() == tree2.getHeight())
						state2 = node.getTypeLabel();
				}

				if (!state1.equals("Anseriformes") || !state2.equals("Anseriformes")) {
					ps.print(c);
					ps.print("\n");
				}
				c += 1;
	...
	// copied from CoalRe
	private Node getSingleChildTree(Network network, int segment, List<String> leafNodes) {
		...
		BitSet root1 = network.getRootEdge().hasSegments;
