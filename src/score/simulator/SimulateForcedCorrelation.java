...
public class SimulateForcedCorrelation extends Network {
...
//	populationFunction = populationFunctionInput.get();
	...
	List<List<NetworkEdge>> remaining;
		HashMap<Double, Set<NetworkEdge>> primedForRea = new HashMap<>();
	do {
...
//			    .getInverseIntensity(transformedTimeToNextCoal + currentTransformedTime) - currentTime;
		...
		if (timeUntilNextEvent == minCoal)
		    coalesce(currentTime, extantLineages, typeIndexCoal);
					...
					reassort(currentTime, extantLineages, typeIndexReassortment, primedForRea);
				else {
					NetworkEdge edge = migrate(currentTime, extantLineages, typeIndexMigrationFrom,
							typeIndexMigrationTo);
					Set<NetworkEdge> edgeList = new HashSet<>();
					edgeList.add(edge);
					primedForRea.put(currentTime + 1.5, edgeList);

//					currentTime += 0.001;
//					reassort(currentTime, extantLineages, typeIndexMigrationTo, edge);
				}
    ...
    private void reassort(double reassortmentTime, HashMap<Integer, List<NetworkEdge>> extantLineages,
			int stateIdReassortment, HashMap<Double, Set<NetworkEdge>> primed) {

		Set<NetworkEdge> result = new HashSet<>();

    	for(Double time : primed.keySet()) {
    		if (time >=reassortmentTime) {
				Set<NetworkEdge> parents = new HashSet<>();
				parents.addAll(primed.get(time));
				int sizeBf = 0;
				int sizeAf = parents.size();

				while (sizeBf < sizeAf) {
					sizeBf = sizeAf;
					for (NetworkEdge e : primed.get(time)) {
						if (e.parentNode != null)
							parents.addAll(e.parentNode.getParentEdges());
					}
					primed.get(time).addAll(parents);
					sizeAf = parents.size();
				}
				result.addAll(primed.get(time));
			}	
    	}
		result.retainAll(extantLineages.get(stateIdReassortment));

		int i = Randomizer.randomChoice(new double[] { 0.75, 0.25 });
		NetworkEdge lineage;
		System.out.println(i);
		if (!result.isEmpty() && i == 0) {
			lineage = new ArrayList<>(result).get((Randomizer.nextInt(result.size())));
		} else
			lineage = extantLineages.get(stateIdReassortment)
	...
	private NetworkEdge migrate(double migrationTime, HashMap<Integer, List<NetworkEdge>> extantLineages,
